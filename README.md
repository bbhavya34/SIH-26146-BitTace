...
## ML Forensic Pipeline

```mermaid
stateDiagram-v2
    [*] --> IDLE
    IDLE --> INGESTING : upload or generate_demo

    INGESTING --> FEATURE_ENGINEERING : DataFrame validated and standardised
    ...
    FEATURE_ENGINEERING --> ANOMALY_DETECTION : Features computed
    note right of FEATURE_ENGINEERING
        velocity burst density (300s window)
        structuring score (0.85t to t)
        fan-in and fan-out counts
        IP diversity per wallet
    end note

    ANOMALY_DETECTION --> GRAPH_BUILDING : Scores assigned
    note right of ANOMALY_DETECTION
        IsolationForest
        n_estimators=100
        contamination=0.15
        random_state=42
    ...
    GRAPH_BUILDING --> CLUSTERING : Graph metrics computed
    note right of GRAPH_BUILDING
        NetworkX DiGraph
        degree_centrality
        betweenness_centrality
        Peel chain detection
        in>=1, out=2, weight ratio>4x
    end note

    CLUSTERING --> RISK_SCORING : Clusters labelled
    note right of CLUSTERING
        DBSCAN
        eps=0.75
        min_samples=3
        features: amount
        burst_score, graph_degree
    end note

    RISK_SCORING --> COMPLETE : Scores and leads written
    note right of RISK_SCORING
        Weighted fusion
        0.30 x anomaly
        0.25 x burst
        0.20 x structuring
        0.15 x peel_chain
        0.10 x graph_centrality
...
```mermaid
flowchart TB
    subgraph PC["Peel Chain — 7 Hop Sequence"]
        direction LR
        PC1["Origin\n24.85 BTC"] -->|"change 24.4 BTC"| PC2["Hop 1"]
        PC1 -->|"peel 0.45 BTC"| M1["Merchant"]
        PC2 -->|"change 24.0 BTC"| PC3["Hop 2"]
        PC2 -->|"peel 0.4 BTC"| M2["Merchant"]
        PC3 -->|"..."| PC4["Hop 7 Terminal"]
    end

    subgraph SM["Fan-In Structuring — Smurfing"]
        direction LR
        S1["Smurf 1\n0.95 BTC"] --> AGG["Aggregator\nWallet"]
        S2["Smurf 2\n0.97 BTC"] --> AGG
        S3["Smurf 3\n0.92 BTC"] --> AGG
        S4["Smurf 4-9\n0.91-0.98 BTC"] --> AGG
    end

    subgraph FO["Fan-Out Layering — Mule Dispersal"]
        direction LR
        ORIGIN["Aggregator\n8.5 BTC"] -->|"0.70 BTC"| ML1["Mule 1"]
        ORIGIN -->|"0.68 BTC"| ML2["Mule 2"]
        ORIGIN -->|"0.72 BTC"| ML3["Mule 3"]
        ORIGIN -->|"..."| ML12["Mule 12"]
    end

    subgraph RB["Rapid Burst — Mixer Interaction"]
        direction LR
        WA["Wallet A"] -->|"2.5 BTC"| WB["Wallet B"]
        WB -->|"2.3 BTC"| WA
        WA -->|"3.1 BTC"| WB
        WB -->|"1.9 BTC"| WA
    end
