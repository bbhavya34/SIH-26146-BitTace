...
export const DashboardPage: React.FC<DashboardPageProps> = ({
  ...
  return (
    <div className="p-6 space-y-6 max-w-7xl mx-auto">
      {/* Real-Time ML Pipeline Status Stepper */}
      <div className="soc-card p-5 bg-soc-900/90 border-soc-800">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-4 border-b border-soc-800/80">
          <div className="flex items-center space-x-3">
            <div className="p-2 rounded-lg bg-soc-800 text-cyan-400 border border-soc-700">
              <Cpu className={`w-5 h-5 ${pipelineState?.is_running ? 'animate-spin text-cyan-400' : ''}`} />
            </div>
            <div>
              <div className="flex items-center space-x-2">
                <h2 className="text-sm font-bold text-white tracking-wide">Multi-Stage Forensic ML Pipeline</h2>
                <span className={`px-2 py-0.5 rounded text-[10px] font-mono font-bold ${
                  pipelineState?.is_running ? 'bg-cyan-950 text-cyan-300 border border-cyan-800 animate-pulse' : 'bg-soc-800 text-emerald-400 border border-soc-700'
                }`}>
                  {pipelineState?.current_stage || 'READY'}
                </span>
              </div>
              <p className="text-xs text-soc-400 mt-0.5 font-mono">
                {pipelineState?.message || 'Pipeline active and waiting for data streams'}
              </p>
            </div>
          </div>
          ...
          <button
            ...
            className="flex items-center space-x-2 px-3.5 py-2 rounded-md bg-cyan-600 hover:bg-cyan-500 active:bg-cyan-700 text-white text-xs font-semibold shadow transition-all disabled:opacity-50 self-start md:self-auto"
          >
            <Sparkles className="w-4 h-4" />
            <span>{isGenerating ? 'Running Pipeline...' : 'Run Pipeline on Synthetic Dataset'}</span>
          </button>
        </div>

        {/* Pipeline Stepper UI */}
        <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-7 gap-2 pt-4">
            ...
            return (
              <div
                ...
                className={`p-2.5 rounded border text-center transition-all ${
                  status === 'active'
                    ? 'bg-cyan-950/60 border-cyan-500/80 text-cyan-300 shadow-[0_0_10px_rgba(6,182,212,0.15)]'
                    : status === 'completed'
                    ? 'bg-soc-950 border-emerald-800/80 text-emerald-400'
